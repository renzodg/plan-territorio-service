...
public class FileWriterService {
  ...
  public byte[] createDatabaseFile(List<DBRecord> dbRecords) throws IOException {
    ...
    extraFields.forEach(extraFieldName -> {
      Cell headerCell = headerRow.createCell(currentHeaderColumnIndex.getAndIncrement());
      headerCell.setCellStyle(headerStyle);
    ...
    dbRecords.forEach(dbRecord -> {
      ...
      dbRecord.getExtraFields().values().forEach(extraFieldValue -> {
  ...
  private void setCellValueSafely(XSSFWorkbook workbook, Cell cell, ExtraFieldValue extraFieldValue) {
